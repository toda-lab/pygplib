    ...
    def read(cls, formula_str: str) -> AbsExpr:
        ...
        * <word> means a non-terminal symbol.
        ...
        The name of a variable symbol is defined as follows::
            ...
            <var_symb>::= <alpha_lower> (<alpha_lower> | <digit> | "_")* "@" <decimal_literal>
            <alpha_lower>::= ["a"-"z"]
            <digit>::= ["0"-"9"]
        ...
        def action_binop(string, location, tokens):
            ...
            accum = cls.binop(tokens[0][1], tokens[0][0], tokens[0][2])
            for i in range(4, len(tokens[0]), 2):
        ...
        # Variable symbols are strings that match [a-z][a-z_0-9]*@[0-9]\+ .
        VAR   = pp.Combine(
            pp.Word(pp.alphas.lower(), pp.alphas.lower()+pp.nums+"_")
            + "@" + pp.Word(pp.nums)).set_parse_action(action_var)
        CONST_REL = (TRUE | FALSE).set_parse_action(action_const)
        ATOM  = VAR | CONST_REL
        ...
        EXPR <<= pp.infix_notation(
            ...
            [
                ...
                (AND,     2, pp.opAssoc.LEFT,  action_binop),
                (OR,      2, pp.opAssoc.LEFT,  action_binop),
