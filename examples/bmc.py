...
from pygplib import NameMgr, Fo
...
class TplCnfParam:
    ...
    """
    def __init__(self, tpl_base: int = 0, tpl_aux_size: int = 0, \
            template_cnf: tuple[tuple[int, ...], ...] = None, \
            target_base: int = 0, target_total: int = 0, \
            is_copied: bool = True):
    ...
    def set_template_cnf(self, template_cnf: tuple[tuple[int, ...], ...]) \
        -> None:
        """Sets template_cnf."""
        self._template_cnf = template_cnf
...
class Bmc:
    """Bounded model checking for graph reconfiguration problem.
        ...
        of first-order variables using graph structure of Fo class.
    """

    def __init__(self, state_expr: Fo, trans_expr: Fo, ini_expr: Fo, \
            fin_expr: Fo, prefix: str = "nx_", trans_type: str = "None"):
        ...
        if trans_expr is None:
            if trans_type == "TJ":
                trans_expr = type(self)._compute_tj_template(self._curr_vars, \
                                                            self._next_vars)
            elif trans_type == "TS":
                trans_expr = type(self)._compute_ts_template(self._curr_vars, \
                                                            self._next_vars)
            else:
        ...
        self._state_size = self._nof_free_vars * Fo.st.get_code_length()
        ...
        expr_tup = ( self._prop_state, ) \
                    + op.compute_domain_constraint(state_expr)
        base, naux, template_cnf = op.compute_cnf( expr_tup )
        # NOTE: the parameters, target_base and target_total, are not yet set,
        # which have to be determined each time cnf is generated.
        self._state_param = TplCnfParam(tpl_base = base, tpl_aux_size = naux, \
                                template_cnf = template_cnf, is_copied = True)
        ...
        # NOTE: the parameters, target_base and target_total, are not yet set,
        # which have to be determined each time cnf is generated.
        self._trans_param = TplCnfParam(tpl_base = base, tpl_aux_size = naux, \
                                template_cnf = template_cnf, is_copied = True)
    ...
    @staticmethod
    def _compute_tj_template(curr_vars, next_vars) -> Fo:
        """Returns first-order formula of Token Jumping."""
        terms = []
        for i,v in enumerate(curr_vars):
            lits = [Fo.neg(Fo.eq(v,w)) for w in next_vars] \
                    + [Fo.eq(curr_vars[j], w) \
                        for j,w in enumerate(next_vars) if i != j]
            res = reduce(Fo.land, lits, Fo.true_const())
            terms.append(res)
        return reduce(Fo.lor, terms, Fo.false_const())


    @staticmethod
    def _compute_ts_template(curr_vars, next_vars) -> Fo:
        """Returns first-order formula of Token Sliding."""
        terms = []
        for i,v in enumerate(curr_vars):
            lits = [Fo.edg(v, next_vars[i])] \
                    + [Fo.neg(Fo.eq(v,w)) for w in next_vars] \
                    + [Fo.eq(curr_vars[j],w) \
                        for j,w in enumerate(next_vars) if i != j]
            res = reduce(Fo.land, lits, Fo.true_const())
            terms.append(res)
    ...
    @staticmethod
    def _generate_cnf_at_time(result_cnf : list[tuple], time: int, \
                    encode_func: Callable[[int, int, TplCnfParam], int], \
                    param: TplCnfParam) -> None:
    ...
    def generate_cnf(self, bound: int, ini_expr: Fo = None, \
                        fin_expr: Fo = None) -> None:
        """Generates CNF."""
        if bound < 0:
        ...
        for time in range(bound+1):
            type(self)._generate_cnf_at_time(fin_cnf, time, \
                                    self._encode_state_func, self._state_param)
        ...
        if 0 < bound:
            ...
            self._trans_aux_base = self._total_state_size \
        ...
        _ini_param = TplCnfParam(tpl_base = base, \
                tpl_aux_size = naux, template_cnf = template_cnf, \
                target_base = self._total_state_size + \
                self._state_param.get_target_total() + \
                self._trans_param.get_target_total() , \
                target_total = naux, is_copied = False)

        self._generate_cnf_at_time(fin_cnf, 0, \
                self._encode_state_func, _ini_param)
        ...
        _fin_param = TplCnfParam(tpl_base = base, \
                tpl_aux_size = naux, template_cnf = template_cnf, \
                target_base = self._total_state_size + \
                self._state_param.get_target_total() + \
                self._trans_param.get_target_total() + \
        ...
        self._generate_cnf_at_time(\
                fin_cnf, bound, self._encode_state_func, _fin_param)
        self.nvar += _fin_param.get_target_total()

        self.cnf  = tuple(fin_cnf)


    def _encode_state_func(self, lit: int, time: int, param: TplCnfParam) \
        -> int:
        ...
        if var > tpl_base: # var is aux
            ...
            symbol_index = Fo.st.get_symbol_index(var)
            assert symbol_index in self._varloc

            new_var = self._varloc[symbol_index] * Fo.st.get_code_length() \
                        + Fo.st.get_code_pos(var) + 1
    ...
    def _encode_trans_func(self, lit: int, time: int, param: TplCnfParam) \
        -> int:
        ...
        if var > tpl_base: # i is aux var
            ...
            if self._curr_vars[self._varloc[symbol_index]] == symbol_index:
                ...
                pos = Fo.st.get_code_pos(var)
                prev_prop_var = Fo.st.get_prop_var(prev_symbol_index, pos)
    ...
    def _decode_symbol_index(self, cnf_lit: int) -> int:
        ...
        var_pos = int( ((cnf_var-1) % self._state_size) \
                        / Fo.st.get_code_length())
    ...
    def _decode_code_pos(self, cnf_lit: int) -> int:
        ...
        code_pos = ( (cnf_var-1) % self._state_size) % Fo.st.get_code_length()
        assert 0 <= code_pos < Fo.st.get_code_length()
        return code_pos
    ...
    def decode(self, assign: tuple) -> tuple:
        ...
        for lit in assign:
            ...
            new_var      = Fo.st.get_prop_var(symbol_index, pos)
            if time not in res:
                res[time] = []
            res[time].append( new_var if lit > 0 else -new_var )

        return tuple(\
                [tuple(res[time]) for time in sorted(res.keys())])


    # Check variable names in state and trans expression
    @staticmethod
    def _check_variable_names_in_expr(state_expr: Fo, \
        trans_expr: Fo, prefix: str):
        ...
        for i in state_vars:
            ...
            if re.match(f"^{prefix}", name):
                raise Exception(f"Variable name {name} in state expression"\
        ...
        if trans_expr is not None:
            ...
            for i in trans_vars:
                ...
                if remain not in state_var_names:
                    raise Exception(f"Variable {name} must not appear"\
                                    +" in trans expression")


    # Check variable names in init state and final state expressions
    @staticmethod
    def _check_variable_names_in_ini_fin_expr(state_vars: tuple[int], \
        ini_expr: Fo, fin_expr: Fo):
        """Checks variable names."""
        state_var_names = [NameMgr.lookup_name(i) for i in state_vars]
        ...
        if ini_expr is not None:
            ...
            ini_var_names = [NameMgr.lookup_name(i) \
                            for i in ini_vars if NameMgr.is_variable(i)]
            ini_con_names = [NameMgr.lookup_name(i) \
                            for i in ini_vars if NameMgr.is_constant(i)]
            assert len(ini_vars) == len(ini_var_names) + len(ini_con_names)

            for name in ini_var_names:
                if name not in state_var_names:
                    raise Exception(f"Variable {name} must not appear "\
                                    +"in init state expression")

            for name in ini_con_names:
                if NameMgr.lookup_index(name) not in \
                                            Fo.st.get_constant_symbol_tuple():
                    raise Exception(f"Cannot intepret constant {name} "
                                    +"as domain element")
        ...
        if fin_expr is not None:
            ...
            # includes not only variables but also constants
            fin_var_names = [NameMgr.lookup_name(i) \
                            for i in fin_vars if NameMgr.is_variable(i)]
            fin_con_names = [NameMgr.lookup_name(i) \
                            for i in fin_vars if NameMgr.is_constant(i)]
