...
def test_compute_cnf():
    ...
    for test_tup, expected in tests:
        ...
        for i in range(1,naux+1):
            assert tab[base+i] == 1 # all indices from base +1 to base+naux are used for aux vars.

        count = 0
        for i in range(1, base+1):
            if tab[i] == 1:
                count += 1
        assert count == expected[0] # some indices from 1 to base are used for propositional variables in test formula.
